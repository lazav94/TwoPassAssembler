...
NOT_instruction::NOT_instruction(int src, int dst) {
	this->src = src;
	this->dst = dst;
...
NOT_instruction::NOT_instruction(string instruction, string name,
...
Elf32_Word NOT_instruction::read_instruction() {
	vector<string> words;
		...
		try {
			...
			cout << "Exception NOT msg: " << exception_msgs[id] << endl;
		} catch (...) {
			cout << "NOT " << exception_msgs[UNKNOWN_EXCEPTION]
					<< " Check NOT instruction!" << endl;
		}
		return 0;
}
