...
AND_instruction::AND_instruction(int src, int dst) {
	this->src = src;
	this->dst = dst;

}

AND_instruction::AND_instruction(string instruction, string name,
...
Elf32_Word AND_instruction::read_instruction() {
	vector<string> words;
	...
	try {
		...
		cout << "Exception AND msg: " << exception_msgs[id] << endl;
	} catch (...) {
		cout << "AND " << exception_msgs[UNKNOWN_EXCEPTION]
				<< " Check AND instruction!" << endl;
	}
	return 0;
}
