...
OUT_instruction::OUT_instruction(int src, int dst, bool flag) {
	this->src = src;
	this->dst = dst;
	this->flag = flag;
...
OUT_instruction::OUT_instruction(string instruction, string name,
...
Elf32_Word OUT_instruction::read_instruction() {
	vector<string> words;
	...
	try {
		...
		cout << "Exception OUT msg: " << exception_msgs[id] << endl;
	} catch (...) {
		cout << "OUT " << exception_msgs[UNKNOWN_EXCEPTION]
				<< " Check OUT instruction!" << endl;
	}
	return 0;
