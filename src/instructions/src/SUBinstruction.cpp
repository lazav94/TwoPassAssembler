...
SUB_instruction::SUB_instruction(int src, int dst, int imm, bool flag) {
	this->src = src;
	this->dst = dst;
	this->imm = imm;
	this->flag = flag;
...
SUB_instruction::SUB_instruction(string instruction, string name,
...
Elf32_Word SUB_instruction::read_instruction() {
	vector<string> words;
		...
		try {
			...
			cout << "Exception SUB msg: " << exception_msgs[id] << endl;
		} catch (...) {
			cout << "SUB " << exception_msgs[UNKNOWN_EXCEPTION]
					<< " Check SUB instruction!" << endl;
		}
		return 0;
