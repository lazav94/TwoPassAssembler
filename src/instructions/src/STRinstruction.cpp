...
STR_instruction::STR_instruction(int a, int r, int f, int imm, bool flag_STR) {
	this->a = a;
	this->r = r;
	this->f = f;
	this->imm = imm;
	this->flag_STR = flag_STR;

}

STR_instruction::STR_instruction(string instruction, string name,
		string condition, bool psw_change, Elf32_Word op_code, int a, int r,
		int f, int imm, bool flag_STR) :
...
STR_instruction::~STR_instruction() {

}

Elf32_Word STR_instruction::read_instruction() {
	vector<string> words;
	...
	try {
		...
		cout << "Exception STR msg: " << exception_msgs[id] << endl;
	} catch (...) {
		cout << "STR " << exception_msgs[UNKNOWN_EXCEPTION]
				<< " Check STR instruction!" << endl;
	}
	return 0;
