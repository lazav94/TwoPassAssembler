...
OR_instruction::OR_instruction(int src, int dst) {
	this->src = src;
	this->dst = dst;
...
OR_instruction::OR_instruction(string instruction, string name,
...
Elf32_Word OR_instruction::read_instruction() {
	vector<string> words;
		...
		try {
			...
			cout << "Exception OR msg: " << exception_msgs[id] << endl;
		} catch (...) {
			cout << "OR " << exception_msgs[UNKNOWN_EXCEPTION]
					<< " Check OR instruction!" << endl;
		}
		return 0;
