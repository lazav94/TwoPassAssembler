...
SHL_instruction::SHL_instruction(int src, int dst, int imm, bool flag_l) {
	this->src = src;
	this->dst = dst;
	this->imm = imm;
	this->flag_l = flag_l;
...
SHL_instruction::SHL_instruction(string instruction, string name,
...
SHL_instruction::~SHL_instruction() {

}

Elf32_Word SHL_instruction::read_instruction() {
	vector<string> words;
	...
	try {
		...
		cout << "Exception SHL msg: " << exception_msgs[id] << endl;
	} catch (...) {
		cout << "LDR " << exception_msgs[UNKNOWN_EXCEPTION]
				<< " Check SHL instruction!" << endl;
	}
	return 0;
