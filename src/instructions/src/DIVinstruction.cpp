...
DIV_instruction::DIV_instruction(int src, int dst, int imm, bool flag) {
	this->src = src;
	this->dst = dst;
	this->imm = imm;
	this->flag = flag;
...
DIV_instruction::DIV_instruction(string instruction, string name,
...
Elf32_Word DIV_instruction::read_instruction() {
	vector<string> words;
		...
		try {
			...
			cout << "Exception DIV msg: " << exception_msgs[id] << endl;
		} catch (...) {
			cout << "DIV " << exception_msgs[UNKNOWN_EXCEPTION]
					<< " Check DIV instruction!" << endl;
		}
		return 0;
